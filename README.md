...
Recebe `nome`, `email` e `password`.

Cria a solicitação de acesso (requer aprovação).

### `POST /api/login`

Recebe `email` e `password`.

Valida o hash da senha e o status de aprovação.

### `GET /docs`

Acessando esta rota no navegador, você abre a documentação interativa automática do Swagger gerada pelo FastAPI.
