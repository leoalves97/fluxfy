    ...
    protegerRota: function (papeisPermitidos) {
        ...
        if (!token || !usuario || !papeisPermitidos.includes(usuario.papel)) {
            ...
            if (usuario && usuario.papel === 'operador') {
                window.location.replace('abertura-caixa.html');
            } else {
        ...
        loginForm.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    let role = "operador"; // Padrão seguro

                    // 1. Verifica se a API retornou o papel explicitamente
                    if (data.papel) {
                        role = (data.papel === 'user') ? 'operador' : data.papel;
                    }

                    // 2. Fallback para a lista de segurança/bypass de admin
                    const emailsAdmin = ['admin@fluxfy.com'];
                    if (emailsAdmin.includes(email)) {
                        role = "admin";
                    }

                    // Salva o usuário logado com o papel correto no localStorage
                    ...
                    window.location.href = role === 'admin' ? 'admin.html' : 'abertura-caixa.html';
                } else {
